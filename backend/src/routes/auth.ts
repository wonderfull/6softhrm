...
function createPasswordResetPayload(user: {
  id: number;
  email: string;
  tokenVersion: number;
}) {
  const secret = getJwtSecret();
  // tokenVersion makes the token single-use: a successful reset bumps it,
  // so a redeemed (or otherwise superseded) token stops matching.
...
router.post('/register', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const employeeMatches = await platformPrisma.employee.findMany({
      where: { email },
    });
    let employee = null;
    if (tenantId) {
    ...
    if (!tenantId) {
      return res.status(403).json({
        error:
          'No employee record found for this email. Ask your HR admin to create your account.',
      });
    }
...
router.post('/login', async (req, res) => {
  ...
  if (
    ...
    return res.status(403).json({
      error:
        'This account is currently unavailable. Contact your administrator.',
    });
  }
  ...
  try {
    ...
    if (user.totpEnabled && user.totpSecret) {
      ...
      const pendingToken = jwt.sign(
        ...
        secret,
        // jti lets /2fa/complete consume the token exactly once.
...
// Pending 2FA tokens are single-use: a completed jti is refused on replay.
// In-memory (per-process, cleared on restart) — bounded by the 5-minute
// token lifetime via pruning on each insert.
const consumedPendingJtis = new Map<string, number>();
function consumePendingJti(jti: string, exp?: number) {
  const now = Date.now();
  for (const [key, expiresAt] of consumedPendingJtis) {
    if (expiresAt <= now) consumedPendingJtis.delete(key);
  }
  consumedPendingJtis.set(jti, exp ? exp * 1000 : now + 5 * 60 * 1000);
}
...
router.post('/2fa/complete', async (req, res) => {
  ...
  try {
    ...
    if (decoded.type !== '2fa-pending' || typeof decoded.jti !== 'string')
      return res.status(401).json({ error: 'Invalid token' });
    if (consumedPendingJtis.has(decoded.jti))
    ...
    // Single-use code check (RFC 6238): a code accepted here cannot be
    // replayed within the ±1-step drift window, even on a fresh pending token.
    if (!verifyTotpOnce(`2fa:${user.id}`, String(code), user.totpSecret)) {
      await createAuditLog(
        user.id,
        user.email,
        'LOGIN_2FA_FAILED',
    ...
    consumePendingJti(decoded.jti, decoded.exp);
...
router.post('/2fa/enable', requireAuth, async (req: any, res) => {
  ...
  if (!verifyTotp(String(code), user.totpSecret)) {
    return res.status(400).json({
      error: 'Invalid authentication code — check your authenticator app',
    });
  }
  await prisma.user.updateMany({
    where: { id: user.id },
...
router.post('/2fa/disable', requireAuth, async (req: any, res) => {
  ...
  if (!code || !verifyTotp(String(code), user.totpSecret)) {
    return res.status(400).json({
      error: 'A valid authentication code is required to disable 2FA',
    });
  }
  await prisma.user.updateMany({
    where: { id: user.id },
...
router.get(
  ...
  async (req, res) => {
    ...
    res.json(
      users.map((user) => ({ ...user, role: normalizeRole(user.role) })),
    );
  },
);
...
router.put(
  ...
  async (req: AuthRequest, res) => {
    ...
    try {
      ...
      if (
        normalizeRole(existingUser.role) === ROLES.ADMIN &&
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (
        ...
        normalizeRole(req.user?.role) !== ROLES.ADMIN
      ) {
        return res.status(403).json({
          error: 'You do not have permission to manage admin accounts',
        });
      ...
      const deliveryAttempted = await sendPasswordResetEmail(
        user.email,
        payload.resetLink,
      );
      await createAuditLog(
      ...
      const status =
        e?.message === 'JWT_SECRET is not configured securely' ? 500 : 400;
      res.status(status).json({
        error: e?.message || 'Failed to generate password reset link',
      });
    }
  },
);
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    const status =
      e?.message === 'JWT_SECRET is not configured securely' ? 500 : 500;
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    // Single-use: the embedded tokenVersion must still match — a successful
    // reset (or any other password change) bumps it, killing the token.
    // Tokens minted without one (tenant setup links) default to 0, which only
    // matches a user whose password has never been reset.
    const user = await platformPrisma.user.findUnique({
      where: { id: decoded.id },
