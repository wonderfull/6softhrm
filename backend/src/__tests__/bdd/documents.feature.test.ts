...
const feature = loadFeature(
  path.join(__dirname, '../features/documents.feature'),
);
...
describe('BDD: documents', () => {
  ...
  defineFeature(feature, (test) => {
    test('Valid document upload succeeds', ({ given, when, then }) => {
      ...
      given('an existing employee record for document upload', async () => {
        const employee = await createEmployee(prefix, {
          email: `${prefix}.upload@example.com`,
        });
        employeeId = employee.id;
        uploadPath = ensureUploadFile(
          prefix,
          `${prefix}-upload.pdf`,
        ).absolutePath;
      });
      ...
      when(
        'an admin uploads a valid PDF document with an expiry date',
        async () => {
      ...
      then(
        'the document is stored with the provided type and expiry date',
        () => {
    ...
    test('Expiring documents returns only documents within the next 30 days', ({
      given,
      when,
      then,
    }) => {
      ...
      given('documents expiring soon and later than 30 days', async () => {
        const employee = await createEmployee(prefix, {
          email: `${prefix}.expiring@example.com`,
        });
        ...
        const soon = await createDocument(employee.id, `${prefix}-soon`, {
          expiryDate: soonDate,
        });
        const later = await createDocument(employee.id, `${prefix}-later`, {
      ...
      then('only the soon-to-expire document is returned', () => {
        expect(response.status).toBe(200);
        expect(
          response.body.some((doc: any) => doc.id === soonDocumentId),
        ).toBe(true);
        expect(
          response.body.some((doc: any) => doc.id === laterDocumentId),
        ).toBe(false);
      });
    });

    test("Employee cannot delete another employee's document", ({
      given,
      when,
      then,
    }) => {
      ...
      given('a document owned by a different employee', async () => {
        const owner = await createEmployee(`${prefix}-owner`, {
          email: `${prefix}.owner@example.com`,
        });
        const requester = await createEmployee(`${prefix}-requester`, {
        ...
        const upload = ensureUploadFile(
          `${prefix}-owner`,
          `${prefix}-owner.pdf`,
        );
        const document = await createDocument(owner.id, `${prefix}-foreign`, {
    ...
    test('Employee cannot delete their own document', ({
      given,
      when,
      then,
    }) => {
      ...
      given('a document owned by the linked employee', async () => {
        ...
        const document = await createDocument(employeeId, `${prefix}-self`, {
          path: upload.publicPath,
        });
        documentId = document.id;
      });
      ...
      then(
        'the own document delete request is rejected with a 403 response',
        () => {
          expect(response.status).toBe(403);
        },
      );
    ...
    test('Download all returns a ZIP for an employee with documents', ({
      given,
      when,
      then,
    }) => {
    ...
    test('Admin can bulk upload payslips for an employee', ({
      given,
      when,
      then,
    }) => {
      ...
      given('an existing employee record for document upload', async () => {
        const employee = await createEmployee(prefix, {
          email: `${prefix}.payslip@example.com`,
        });
        employeeId = employee.id;
        uploadPathOne = ensureUploadFile(
          prefix,
          `${prefix}-payslip-one.pdf`,
        ).absolutePath;
        uploadPathTwo = ensureUploadFile(
          prefix,
          `${prefix}-payslip-two.pdf`,
        ).absolutePath;
      });
      ...
      then(
        'the uploaded payslips are stored without public share links',
        () => {
          expect(response.status).toBe(200);
