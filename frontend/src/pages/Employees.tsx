...
function EmployeeInput({
  ...
  return (
    <div className={className}>
      <label
        htmlFor={fieldId}
        className="mb-1 block text-sm font-semibold text-slate-800 dark:text-slate-200"
      >
        {label}
...
export default function Employees() {
  ...
  const pendingReviews = employees.filter(
    (employee) => (employee.consentCount ?? 7) < 3,
  ).length;
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 dark:border-slate-700 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {!canManageEmployees &&
            selectedEmployee &&
            canEditOwnProfile(selectedEmployee) && (
              <button
                type="button"
      ...
      {status && (
        <div className="flex items-center justify-between rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200">
          ...
          <button
            ...
            className="rounded p-1 hover:bg-emerald-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:hover:bg-emerald-900"
          >
            <HiXMark size={18} />
          </button>
        </div>
              ...
              <div className="flex flex-col gap-3 border-b border-slate-200 p-3 dark:border-slate-700 sm:flex-row">
                ...
                <div className="relative flex-1">
                  <input
                    ...
                    className="form-input min-h-10 py-2 pr-9 w-full"
                  ...
                  {query && (
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-700"
                    >
                      <HiXMark size={16} />
                    </button>
                  )}
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {visibleEmployees.map((employee) => {
                      ...
                      const role = normalizeRole(
                        account?.role || employee.employeeType,
                      );
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            {account ? (
                              ...
                              <Badge className="border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-800 dark:bg-amber-900/30 dark:text-amber-200">
                                NO LOGIN
                              </Badge>
                            )}
                          </td>
                          ...
                          <td className="px-4 py-3">
                            {employee.consentCount === undefined ? (
                              ...
                              <Badge className="border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-800 dark:bg-amber-900/30 dark:text-amber-200">
                                {employee.consentCount}/7 consents
                              </Badge>
                            ) : (
                              <Badge className="border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200">
                                {employee.consentCount}/7 consents
                              </Badge>
                            )}
                          </td>
                              ...
                              {canManageAccounts &&
                                ...
                                canAssignRole(currentRole, role) && (
                                  <button
                                    ...
                                    onClick={() =>
                                      openAccountForm(employee, account)
                                    }
                ...
                <>
                  <div className="border-b border-slate-200 p-5 dark:border-slate-700">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {selectedAccount ? (
                        <Badge className={roleBadgeClass(selectedAccount.role)}>
                  ...
                  <dl className="grid grid-cols-1 gap-4 border-b border-slate-200 p-5 dark:border-slate-700 sm:grid-cols-2 xl:grid-cols-1">
                    ...
                    <Field
                      label="Mobile"
                      value={selectedEmployee.phoneNumber || 'Not provided'}
                    />
                    <Field
                      label="Work Phone"
                      value={selectedEmployee.workPhone || 'Not provided'}
                    />
                    <Field
                      label="Start Date"
                    ...
                    <Field
                      ...
                      value={
                        [
                          ...
                          selectedEmployee.address2,
                          selectedEmployee.townCity,
                    ...
                    {canViewSensitive && (
                      <>
                        <Field
                          label="Tax Code"
                          value={selectedEmployee.taxCode || 'Not provided'}
                        />
                        <Field
                          label="NI Number"
                          value={selectedEmployee.niNumber || 'Not provided'}
                        />
                        <Field
                          label="Bank"
                          value={selectedEmployee.bankName || 'Not provided'}
                        />
                        <Field
                          label="Account"
                        ...
                        <Field
                          label="Sort Code"
                          value={selectedEmployee.sortCode || 'Not provided'}
                        />
                        <Field
                          label="Salary"
                          value={
                        ...
                        <Field
                          label="Payroll"
                          value={
                            selectedEmployee.payrollNumber || 'Not provided'
                          }
                        />
                        <Field
                          label="Visa Expiry"
                  ...
                  <div className="border-b border-slate-200 p-5 dark:border-slate-700">
                    ...
                    {selectedAccount ? (
                      <div className="space-y-2">
                        ...
                        {canManageAccounts &&
                          canAssignRole(currentRole, selectedAccount.role) && (
                            <div className="grid gap-2">
                              <button
                                ...
                                className="btn-ghost min-h-10 justify-center"
                              >
                                <HiPencilSquare size={17} />
                                Edit Account
                              </button>
                              <button
                                type="button"
                                onClick={() =>
                                  handleGenerateResetLink(selectedAccount)
                                }
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  openTempPasswordDialog(selectedAccount)
                                }
                                disabled={busyAccountId === selectedAccount.id}
                                className="btn-ghost min-h-10 justify-center"
                              >
                      ...
                      <button
                        type="button"
                        onClick={() => openAccountForm(selectedEmployee)}
                        className="btn-primary min-h-10 w-full justify-center"
                      >
                        <HiUserPlus size={17} />
                        Create Account
                      </button>
        ...
        <section className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/50">
          <div className="mb-4 flex flex-col gap-3 border-b border-slate-200 pb-4 dark:border-slate-700 sm:flex-row sm:items-start sm:justify-between">
            ...
            <button
              ...
              className="rounded-md p-2 text-slate-500 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-primary-400 dark:hover:bg-slate-700"
            >
              <HiXMark size={20} />
            </button>
          </div>
          ...
          >
            ...
            >
              ...
              <EmployeeInput
                ...
                value={employeeForm.emergencyContactAddress}
                onChange={updateEmployeeField}
                placeholder="Address"
              />
            </FormSection>
            ...
            {!isSelfProfileEdit && (
              <FormSection title="Salary details">
                ...
                <EmployeeInput
                  ...
                  value={employeeForm.salaryEffectiveFrom}
                  onChange={updateEmployeeField}
                  type="date"
                />
                <EmployeeInput
                  id="salaryReason"
                  label="Reason"
                  value={employeeForm.salaryReason}
                  onChange={updateEmployeeField}
                  options={[
            ...
            {!isSelfProfileEdit && (
              ...
              >
                ...
                <EmployeeInput
                  ...
                  value={employeeForm.passportCountryOfIssue}
                  onChange={updateEmployeeField}
                ...
                <EmployeeInput
                  ...
                  value={employeeForm.passportExpiryDate}
                  onChange={updateEmployeeField}
                  type="date"
                />
                <div className="md:col-span-2">
                ...
                <EmployeeInput
                  ...
                  value={employeeForm.licenceNumber}
                  onChange={updateEmployeeField}
                ...
                <EmployeeInput
                  ...
                  label="Country of issue"
                  value={employeeForm.licenceCountryOfIssue}
                  onChange={updateEmployeeField}
                  placeholder="Country of issue"
                />
                <EmployeeInput
                  id="licenceClass"
                  label="Licence class"
                  value={employeeForm.licenceClass}
                  onChange={updateEmployeeField}
                ...
                <EmployeeInput
                  ...
                  value={employeeForm.licenceExpiryDate}
                  onChange={updateEmployeeField}
                  type="date"
                />
                <div className="md:col-span-2">
        ...
        >
          <div className="mb-4 flex items-center justify-between gap-3">
            <h3 className="text-lg font-semibold">
              {accountForm.id ? 'Edit Account' : 'New Account'}
            </h3>
            <button
              type="button"
          ...
          >
              ...
              >
                {assignableRoles(currentRole).map((role) => (
                  <option key={role} value={role}>
                    {accountForm.employeeId && role !== 'EMPLOYEE'
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/50">
              <label
                ...
                className="flex items-start gap-3 text-sm font-semibold text-slate-800 dark:text-slate-100"
              >
                <input
                ...
                <span>
                  ...
                  <span className="mt-1 block text-xs font-normal leading-5 text-slate-500 dark:text-slate-400">
                    Check this to make the account both{' '}
                    {roleLabel(accountForm.role)} and employee-linked. Leave
                    unchecked for {roleLabel(accountForm.role)} access only.
                  </span>
                </span>
              </label>
            </div>
            <div className="flex gap-2 md:col-span-2">
              <button
                type="submit"
                className="btn-primary min-h-10 flex-1 justify-center"
              >
      ...
      <Dialog
        ...
        description={
          tempPasswordDialog
            ? `Generates a one-time password for ${tempPasswordDialog.account.email}. Copy it before closing — it isn't shown again.`
            : ''
        }
        onClose={() =>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                disabled={savingTempPassword}
              >
                {savingTempPassword ? 'Saving…' : 'Apply password'}
              </button>
            </div>
          </div>
        )}
