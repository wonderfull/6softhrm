...
function chipClass(entry: Entry) {
 ...
 return 'border-warn bg-warn-tint text-warn ';
 if (entry.type === 'SICK')
...
export default function LeaveCalendar() {
 ...
 className="rounded-md border border-line-2 px-3 py-1 text-sm font-semibold hover:bg-surface-2 "
          >
            ‹
          </button>
 ...
 className="form-input w-auto"
      ...
      <div className="overflow-x-auto">
        <div className="grid min-w-[45rem] grid-cols-7 gap-px rounded-lg bg-surface-3 ">
          {WEEKDAYS.map((name) => (
            <div
 key={name}
 className="bg-surface-2 px-2 py-1 text-center text-xs font-semibold uppercase tracking-wide text-ink-2 "
            >
              {name}
            </div>
          ))}
 ...
 className={`min-h-24 p-1 ${ isBankHoliday
                    ? 'bg-warn-tint'
                    : isWeekend
                      ? 'bg-surface-2 '
                      : 'bg-surface'
