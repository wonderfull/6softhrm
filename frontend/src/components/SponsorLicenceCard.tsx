...
function PersonList({
 ...
 className="text-xs text-ink-2 hover:underline dark:text-ink-2"
          >
            + Add
          </button>
        )}
      </div>
      {people.length === 0 && <p className="mt-1 text-xs text-ink-3">None recorded.</p>}
...
export default function SponsorLicenceCard({ canEdit }: { canEdit: boolean }) {
 ...
 expiry, action plan and CoS start-by dates feed the reminder emails.
      ...
      {expiryDays !== null && expiryDays <= 90 && (
        <div className="mb-3 rounded-md border border-warn bg-warn-tint px-3 py-2 text-sm text-warn ">
          {expiryDays < 0
