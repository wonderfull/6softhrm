          ...
          <div className="flex flex-col gap-1">
            {menu.map((m) => (
              <Link
                key={m.to}
                to={m.to}
                className={`flex items-center gap-3 p-3 rounded-md ${loc.pathname === m.to ? 'bg-primary-50 dark:bg-primary-900 text-primary-700 dark:text-primary-300' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
              >
