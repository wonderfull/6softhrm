...
describe('Sponsorships Page', () => {
  ...
  beforeEach(() => {
    ...
    (api.apiGet as any).mockImplementation((endpoint: string) => {
      if (endpoint === '/sponsorships')
        return Promise.resolve(mockSponsorships);
      if (endpoint === '/employees') return Promise.resolve(mockEmployees);
      if (endpoint === '/sponsorships/reportable-events/open')
      ...
      if (endpoint === '/sponsorships/1/compliance')
        return Promise.resolve({
          sponsorship: { id: 1 },
          employee: mockEmployees[0],
          missingCount: 4,
  ...
  it('should submit new sponsorship with all required fields', async () => {
    ...
    fireEvent.change(await screen.findByLabelText(/Employee/i), {
      target: { value: '1' },
    });
    fireEvent.change(screen.getByLabelText(/Visa Type/i), {
  ...
  it('uploads and links missing compliance evidence for the sponsored employee', async () => {
    ...
    fireEvent.click(
      screen.getByRole('button', {
        name: /Add evidence for Recruitment evidence/i,
      }),
    );
    ...
    fireEvent.change(screen.getByLabelText(/Evidence file/i), {
      target: {
        files: [
          new File(['evidence'], 'recruitment.pdf', {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: /Upload and link evidence/i }),
    );
    ...
    await waitFor(() => {
      ...
      expect(api.apiPost).toHaveBeenCalledWith(
        '/sponsorships/1/compliance/evidence',
        {
  ...
  it('shows linked evidence and allows replacing an existing upload', async () => {
    (api.apiGet as any).mockImplementation((endpoint: string) => {
      if (endpoint === '/sponsorships')
        return Promise.resolve(mockSponsorships);
      if (endpoint === '/employees') return Promise.resolve(mockEmployees);
          ...
          requiredEvidence: [
            {
              ...
              label: 'Right-to-work check',
              status: 'COMPLETE',
              evidence: { notes: '', document: { name: 'RTW - John Doe' } },
            },
          ],
          existingEvidence: [],
        });
    ...
    await screen.findByText(/Right-to-work check/i);
    // Issue 1: linked evidence with no notes must not read "Evidence not linked"
    expect(screen.queryByText(/Evidence not linked/i)).not.toBeInTheDocument();
    expect(screen.getByText(/Linked: RTW - John Doe/i)).toBeInTheDocument();

    // Issue 2: a Replace evidence control is available for completed items
    fireEvent.click(
      screen.getByRole('button', {
        name: /Replace evidence for Right-to-work check/i,
      }),
    );
    expect(
      screen.getByRole('button', { name: /Replace and re-link evidence/i }),
    ).toBeInTheDocument();
  });
  ...
  it('hides core export and edit actions from office assistants', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: /Add Sponsorship/i }),
    ).not.toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByRole('button', { name: /Add reportable event/i }),
    ).toBeInTheDocument();
  });
  ...
  it('does not mark synthetic reportable alerts as reported', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: /Mark reported/i }),
    ).not.toBeInTheDocument();
  });
});
