...
vi.mock('../lib/api', async (importOriginal) => {
  ...
  return {
    ...
    hasRole: vi.fn(
      (user: any, ...roles: string[]) => !!user && roles.includes(user.role),
    ),
  };
});
...
describe('Documents Page', () => {
  ...
  it('should call fetch when admin clicks Download All', async () => {
    ...
    const mockRes = {
      ...
      headers: { get: () => 'attachment; filename="John_Doe_Documents.zip"' },
    };
    const fetchSpy = vi
  ...
  it('should call fetch when employee clicks Download All My Documents', async () => {
    ...
    localStorage.setItem(
      'token',
      makeToken({ role: 'USER', email: 'john@test.com', employeeId: 1 }),
    );
    const mockRes = {
      ok: true,
      blob: async () => new Blob(['zip content']),
    ...
    fireEvent.click(
      await screen.findByText(/Download All My Documents as ZIP/i),
    );
  ...
  it('should show download all button for employee user', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Download All My Documents as ZIP/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it('should display document type badges', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('heading', { name: /Employment Contracts/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole('heading', { name: /Passports/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole('heading', { name: /Payslips/i }),
      ).toBeInTheDocument();
  ...
  it('should render the payslip drop zone for admins', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Drag and drop payslips here/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it('should display expiry dates with proper formatting', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryAllByText(/\d{1,2}\/\d{1,2}\/\d{4}/).length,
      ).toBeGreaterThan(0);
    });
  });
  ...
  it('does not expose any share-link controls to admins (feature removed)', async () => {
    mockedUser = { role: 'ADMIN', email: 'admin@example.com' };
    localStorage.setItem(
      'token',
      makeToken({ role: 'ADMIN', email: 'admin@example.com' }),
  ...
  it('previews a PDF document inline', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: /Preview Contract.pdf/i }),
    );
  ...
  it('previews a PDF when the display name has no extension but the stored path does', async () => {
    ...
    const objectUrlSpy = vi
      .spyOn(window.URL, 'createObjectURL')
      .mockReturnValue('blob:payslip-preview-url');

    renderDocuments();

    fireEvent.click(
      await screen.findByRole('button', { name: /Preview 6 SOFT LIMITED/i }),
    );
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTitle(/6 SOFT LIMITED - Employee Payslip/i),
      ).toHaveAttribute('src', 'blob:payslip-preview-url');
    });
    expect(
  ...
  it('downloads a document with the original file name', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: /Download Contract.pdf/i }),
    );
  ...
  it('opens Word documents in the preview frame instead of showing unsupported file type', async () => {
    ...
    const objectUrlSpy = vi
      .spyOn(window.URL, 'createObjectURL')
      .mockReturnValue('blob:docx-preview-url');

    renderDocuments();

    fireEvent.click(
      await screen.findByRole('button', { name: /Preview Policy.docx/i }),
    );

    const dialog = await screen.findByRole('dialog', {
      name: /Preview Policy.docx/i,
    });
    expect(within(dialog).getByTitle('Policy.docx')).toHaveAttribute(
      'src',
      'blob:docx-preview-url',
    );
    expect(
      within(dialog).queryByText(
        /Preview is not available for this file type/i,
      ),
    ).not.toBeInTheDocument();
    expect(
      within(dialog).getByRole('button', { name: /Download Policy.docx/i }),
    ).toBeInTheDocument();
  ...
  it('hides delete controls from office assistants', async () => {
    mockedUser = { role: 'OFFICE_ASSISTANT', email: 'assistant@example.com' };
    localStorage.setItem(
      'token',
      makeToken({ role: 'OFFICE_ASSISTANT', email: 'assistant@example.com' }),
    );
  ...
  it('should filter documents by employee', async () => {
    ...
    fireEvent.change(await screen.findByLabelText(/Selected Employee/i), {
      target: { value: '1' },
    });
  ...
  it('requires admins to select an employee before documents are listed', async () => {
    ...
    expect(
      await screen.findByText(/Select an employee to view their documents/i),
    ).toBeInTheDocument();
